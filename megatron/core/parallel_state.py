...
class ParallelState:
    def __init__(self):
        ...
        self._DATA_PARALLEL_GROUP_GLOO = None
        # tensor model parallel group and data parallel group combined
        # used for fp8 and moe training
        self._TENSOR_AND_DATA_PARALLEL_GROUP = None
        # Expert parallel group that the current rank belongs to.
        self._TENSOR_AND_EXPERT_PARALLEL_GROUP = None
        self._DATA_MODULO_EXPERT_PARALLEL_GROUP = None
        ...
        self._MPU_EXPERT_MODEL_PARALLEL_WORLD_SIZE = None
        self._MPU_TENSOR_MODEL_PARALLEL_RANK = None
        self._MPU_PIPELINE_MODEL_PARALLEL_RANK = None
        self._MPU_EXPERT_MODEL_PARALLEL_RANK = None
        ...
        self._CONTEXT_PARALLEL_GLOBAL_RANKS = None

        # Data parallel group information with context parallel combined.
        self._DATA_PARALLEL_GROUP_WITH_CP = None
        self._DATA_PARALLEL_GROUP_WITH_CP_GLOO = None
        self._DATA_PARALLEL_GLOBAL_RANKS_WITH_CP = None

        # combined parallel group of TP, DP, and CP used for fp8
        self._TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP = None

        # Memory buffers to avoid dynamic memory allocation
        self._GLOBAL_MEMORY_BUFFER = None


    def get_nccl_options(self, pg_name, nccl_comm_cfgs):
    ...
    def initialize_model_parallel(
        ...
        """***IMPORTANT***:
            context parallel and expert parallel not yet supported
        """
        ...
        if model_world_size is not None:
        ...
        if (
            world_size
            % (tensor_model_parallel_size * pipeline_model_parallel_size) #don't support context parallel yet* context_parallel_size)
            != 0
        ):
            raise RuntimeError(
        ...
        for i in range(pipeline_model_parallel_size):
            ...
            for j in range(context_parallel_size * tensor_model_parallel_size):
                ...
                group = torch.distributed.new_group(
                    ranks, pg_options=self.get_nccl_options('dp', nccl_comm_cfgs)
                )
                group_gloo = torch.distributed.new_group(ranks, backend="gloo")
                if rank in ranks:
                    self._DATA_PARALLEL_GROUP = group
                    self._DATA_PARALLEL_GROUP_GLOO = group_gloo
                    self._DATA_PARALLEL_GLOBAL_RANKS = ranks
            for j in range(tensor_model_parallel_size):
                ranks_with_cp = range(start_rank + j, end_rank, tensor_model_parallel_size)
                all_data_parallel_group_ranks_with_cp.append(list(ranks_with_cp))
                group_with_cp = torch.distributed.new_group(
                    ranks_with_cp, pg_options=self.get_nccl_options('dp_cp', nccl_comm_cfgs)
                )
                group_with_cp_gloo = torch.distributed.new_group(ranks_with_cp, backend="gloo")
                if rank in ranks_with_cp:
                    self._DATA_PARALLEL_GROUP_WITH_CP = group_with_cp
                    self._DATA_PARALLEL_GROUP_WITH_CP_GLOO = group_with_cp_gloo
                    self._DATA_PARALLEL_GLOBAL_RANKS_WITH_CP = ranks_with_cp
        ...
        if use_sharp:
            ...
            torch.distributed.barrier(
                group=self.get_data_parallel_group(with_context_parallel=context_parallel_size > 1),
        ...
        assert self._CONTEXT_PARALLEL_GROUP is None, 'context parallel group is already initialized'
        for i in range(pipeline_model_parallel_size):
            for j in range(data_parallel_size):
                ...
                for k in range(tensor_model_parallel_size):
                    ...
                    group = torch.distributed.new_group(
                        ranks, pg_options=self.get_nccl_options('cp', nccl_comm_cfgs)
                    )
                    if rank in ranks:
                        self._CONTEXT_PARALLEL_GROUP = group
                        self._CONTEXT_PARALLEL_GLOBAL_RANKS = ranks

        # Build the model-parallel groups.
        assert self._MODEL_PARALLEL_GROUP is None, 'model parallel group is already initialized'
        for i in range(data_parallel_size * context_parallel_size):
            ...
            group = torch.distributed.new_group(
                ranks, pg_options=self.get_nccl_options('mp', nccl_comm_cfgs)
            )
            if rank in ranks:
        ...
        for i in range(num_tensor_model_parallel_groups):
            ...
            group = torch.distributed.new_group(
                ranks, pg_options=self.get_nccl_options('tp', nccl_comm_cfgs)
            )
            if rank in ranks:
        ...
        assert self._EMBEDDING_GROUP is None, 'embedding group is already initialized'
        # global _POSITION_EMBEDDING_GROUP
        # global _POSITION_EMBEDDING_GLOBAL_RANKS
        assert self._POSITION_EMBEDDING_GROUP is None, 'position embedding group is already initialized'
        for i in range(num_pipeline_model_parallel_groups):
            ranks = range(i, world_size, num_pipeline_model_parallel_groups)
            group = torch.distributed.new_group(
                ranks, pg_options=self.get_nccl_options('pp', nccl_comm_cfgs)
            )
            if rank in ranks:
            ...
            group = torch.distributed.new_group(
                embedding_ranks, pg_options=self.get_nccl_options('embd', nccl_comm_cfgs)
            )
            if rank in embedding_ranks:
                self._EMBEDDING_GROUP = group
            if rank in ranks:
                self._EMBEDDING_GLOBAL_RANKS = embedding_ranks

            group = torch.distributed.new_group(
                position_embedding_ranks, pg_options=self.get_nccl_options('embd', nccl_comm_cfgs)
            )
            if rank in position_embedding_ranks:
                self._POSITION_EMBEDDING_GROUP = group
            if rank in ranks:
                self._POSITION_EMBEDDING_GLOBAL_RANKS = position_embedding_ranks

        # Build the tensor + data parallel groups.
        # global _TENSOR_AND_DATA_PARALLEL_GROUP
        # global _TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP
        assert (
            self._TENSOR_AND_DATA_PARALLEL_GROUP is None
        ...
        for i in range(num_tensor_and_data_groups_with_cp):
            ...
            group = torch.distributed.new_group(
                ranks, pg_options=self.get_nccl_options('tp_dp_cp', nccl_comm_cfgs)
            )
            if rank in ranks:
                self._TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP = group
            ...
            for j in range(context_parallel_size):
                ...
                group = torch.distributed.new_group(
                    ranks, pg_options=self.get_nccl_options('tp_dp', nccl_comm_cfgs)
                )
                if rank in ranks:
                    self._TENSOR_AND_DATA_PARALLEL_GROUP = group

        # Build the tensor + expert parallel groups
        # global _TENSOR_AND_EXPERT_PARALLEL_GROUP
        assert (
            self._TENSOR_AND_EXPERT_PARALLEL_GROUP is None
        ), 'Tensor + expert parallel group is already initialized'
        # global _DATA_MODULO_EXPERT_PARALLEL_GROUP
        assert (
            self._DATA_MODULO_EXPERT_PARALLEL_GROUP is None
        ...
        for i in range(num_tensor_and_data_groups):
            for j in range(num_expert_groups):
                ...
                ranks = range(start_rank, end_rank)
                group = torch.distributed.new_group(
                    ranks, pg_options=gself.et_nccl_options('tp_exp', nccl_comm_cfgs)
                )
                if rank in ranks:
                    self._TENSOR_AND_EXPERT_PARALLEL_GROUP = group
        ...
        for i in range(num_tensor_and_data_groups):
            ...
            for j in range(tensor_and_expert_group_size):
                ...
                group = torch.distributed.new_group(
                    ranks, pg_options=self.get_nccl_options('dp_modulo_exp', nccl_comm_cfgs)
                )
                if rank in ranks:
                    self._DATA_MODULO_EXPERT_PARALLEL_GROUP = group
    ...
    def is_unitialized(self):
        ...
        return self._DATA_PARALLEL_GROUP is None
    ...
    def get_model_parallel_group(self):
        ...
        assert self._MODEL_PARALLEL_GROUP is not None, 'model parallel group is not initialized'
    ...
    def get_data_parallel_group(self, with_context_parallel=False):
        ...
        if with_context_parallel:
            assert (
                self._DATA_PARALLEL_GROUP_WITH_CP is not None
            ), 'data parallel group with context parallel combined is not initialized'
            return self._DATA_PARALLEL_GROUP_WITH_CP
        else:
    ...
    def get_data_parallel_group_gloo(self, with_context_parallel=False):
        ...
        if with_context_parallel:
            assert (
                self._DATA_PARALLEL_GROUP_WITH_CP_GLOO is not None
            ), 'data parallel group-gloo with context parallel combined is not initialized'
            return self._DATA_PARALLEL_GROUP_WITH_CP_GLOO
        else:
            assert self._DATA_PARALLEL_GROUP_GLOO is not None, 'data parallel group-gloo is not initialized'
            return self._DATA_PARALLEL_GROUP_GLOO


    def get_context_parallel_group(self, check_initialized=True):
        """Get the context parallel group the caller rank belongs to."""
        if check_initialized:
            assert self._CONTEXT_PARALLEL_GROUP is not None, 'context parallel group is not initialized'
    ...
    def get_context_parallel_global_ranks(self, check_initialized=True):
        ...
        if check_initialized:
            assert (
                self._CONTEXT_PARALLEL_GLOBAL_RANKS is not None
            ), 'context parallel group is not initialized'
        return self._CONTEXT_PARALLEL_GLOBAL_RANKS


    def get_embedding_group(self):
        """Get the embedding group the caller rank belongs to."""
        assert self._EMBEDDING_GROUP is not None, 'embedding group is not initialized'
        return self._EMBEDDING_GROUP


    def get_position_embedding_group(self):
        """Get the position embedding group the caller rank belongs to."""
        assert self._POSITION_EMBEDDING_GROUP is not None, 'position embedding group is not initialized'
        return self._POSITION_EMBEDDING_GROUP
    ...
    def get_amax_reduction_group(self, with_context_parallel=False):
        ...
        if with_context_parallel:
            assert (
                self._TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP is not None
            ), 'FP8 amax reduction group is not initialized'
            return self._TENSOR_AND_DATA_PARALLEL_GROUP_WITH_CP
        else:
            assert (
                self._TENSOR_AND_DATA_PARALLEL_GROUP is not None
            ), 'FP8 amax reduction group is not initialized'
            return self._TENSOR_AND_DATA_PARALLEL_GROUP
    ...
    def get_tensor_and_expert_parallel_group(self):
        assert (
            self._TENSOR_AND_EXPERT_PARALLEL_GROUP is not None
        ), 'tensor and expert parallel group is not initialized'
        return self._TENSOR_AND_EXPERT_PARALLEL_GROUP


    def get_data_modulo_expert_parallel_group(self):
        assert (
            self._DATA_MODULO_EXPERT_PARALLEL_GROUP is not None
        ), 'data modulo expert parallel group is not initialized'
        return self._DATA_MODULO_EXPERT_PARALLEL_GROUP


    def set_expert_model_parallel_world_size(self, world_size):
        # global _MPU_EXPERT_MODEL_PARALLEL_WORLD_SIZE
        self._MPU_EXPERT_MODEL_PARALLEL_WORLD_SIZE = world_size


    def set_tensor_model_parallel_world_size(self, world_size):
        """Set the tensor model parallel size"""
        # global _MPU_TENSOR_MODEL_PARALLEL_WORLD_SIZE
        self._MPU_TENSOR_MODEL_PARALLEL_WORLD_SIZE = world_size


    def set_pipeline_model_parallel_world_size(self, world_size):
        """Set the pipeline model parallel size"""
        # global _MPU_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
        self._MPU_PIPELINE_MODEL_PARALLEL_WORLD_SIZE = world_size


    def set_virtual_pipeline_model_parallel_world_size(self, world_size):
        """Set the pipeline model parallel size"""
        # global _VIRTUAL_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
        self._VIRTUAL_PIPELINE_MODEL_PARALLEL_WORLD_SIZE = world_size


    def get_tensor_model_parallel_world_size(self):
        """Return world size for the tensor model parallel group."""
        # global _MPU_TENSOR_MODEL_PARALLEL_WORLD_SIZE
        if self._MPU_TENSOR_MODEL_PARALLEL_WORLD_SIZE is not None:
            return self._MPU_TENSOR_MODEL_PARALLEL_WORLD_SIZE
    ...
    def get_pipeline_model_parallel_world_size(self):
        ...
        # global _MPU_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
        if self._MPU_PIPELINE_MODEL_PARALLEL_WORLD_SIZE is not None:
            return self._MPU_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
    ...
    def set_expert_model_parallel_rank(self, rank):
        ...
        self._MPU_EXPERT_MODEL_PARALLEL_RANK = rank


    def set_tensor_model_parallel_rank(self, rank):
        """Set tensor model parallel rank."""
        # global _MPU_TENSOR_MODEL_PARALLEL_RANK
        self._MPU_TENSOR_MODEL_PARALLEL_RANK = rank


    def set_pipeline_model_parallel_rank(self, rank):
        """Set pipeline model parallel rank."""
        # global _MPU_PIPELINE_MODEL_PARALLEL_RANK
        self._MPU_PIPELINE_MODEL_PARALLEL_RANK = rank
    ...
    def get_tensor_model_parallel_rank(self):
        ...
        # global _MPU_TENSOR_MODEL_PARALLEL_RANK
        if self._MPU_TENSOR_MODEL_PARALLEL_RANK is not None:
            return self._MPU_TENSOR_MODEL_PARALLEL_RANK
    ...
    def get_pipeline_model_parallel_rank(self):
        ...
        # global _MPU_PIPELINE_MODEL_PARALLEL_RANK
        if self._MPU_PIPELINE_MODEL_PARALLEL_RANK is not None:
            return self._MPU_PIPELINE_MODEL_PARALLEL_RANK
    ...
    def is_pipeline_last_stage(self, ignore_virtual=False):
        ...
        if not ignore_virtual:
            ...
            if virtual_pipeline_model_parallel_world_size is not None and self.get_virtual_pipeline_model_parallel_rank() != (
    ...
    def is_rank_in_embedding_group(self, ignore_virtual=False):
        ...
        # global _EMBEDDING_GLOBAL_RANKS
        if ignore_virtual:
            return rank in self._EMBEDDING_GLOBAL_RANKS
        if rank in self._EMBEDDING_GLOBAL_RANKS:
            if rank == self._EMBEDDING_GLOBAL_RANKS[0]:
                return self.is_pipeline_first_stage(ignore_virtual=False)
            elif rank == self._EMBEDDING_GLOBAL_RANKS[-1]:
    ...
    def is_rank_in_position_embedding_group(self):
        ...
        rank = torch.distributed.get_rank()
        # global _POSITION_EMBEDDING_GLOBAL_RANKS
        return rank in self._POSITION_EMBEDDING_GLOBAL_RANKS
    ...
    def is_pipeline_stage_before_split(self, rank=None):
        ...
        # global _PIPELINE_MODEL_PARALLEL_SPLIT_RANK
        if self._PIPELINE_MODEL_PARALLEL_SPLIT_RANK is None:
            return True
        if rank < self._PIPELINE_MODEL_PARALLEL_SPLIT_RANK:
            return True
        return False
    ...
    def is_pipeline_stage_at_split(self):
        ...
        decoder."""
        rank = self.get_pipeline_model_parallel_rank()
        return self.is_pipeline_stage_before_split(rank) and self.is_pipeline_stage_after_split(rank + 1)
    ...
    def get_virtual_pipeline_model_parallel_world_size(self):
        ...
        # global _VIRTUAL_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
        return self._VIRTUAL_PIPELINE_MODEL_PARALLEL_WORLD_SIZE
    ...
    def get_data_parallel_src_rank(self, with_context_parallel=False):
        ...
        if with_context_parallel:
            assert (
                self._DATA_PARALLEL_GLOBAL_RANKS_WITH_CP is not None
            ), "Data parallel group with context parallel combined is not initialized"
            return self._DATA_PARALLEL_GLOBAL_RANKS_WITH_CP[0]
        else:
            assert self._DATA_PARALLEL_GLOBAL_RANKS is not None, "Data parallel group is not initialized"
            return self._DATA_PARALLEL_GLOBAL_RANKS[0]


    def get_pipeline_model_parallel_first_rank(self):
        """Return the global rank of the first process in the pipeline for the
        current tensor parallel group"""
        assert self._PIPELINE_GLOBAL_RANKS is not None, "Pipeline parallel group is not initialized"
    ...
    def get_pipeline_model_parallel_last_rank(self):
        ...
        return self._PIPELINE_GLOBAL_RANKS[last_rank_local]
    ...
    def get_pipeline_model_parallel_next_rank(self):
        ...
        return self._PIPELINE_GLOBAL_RANKS[(rank_in_pipeline + 1) % world_size]
    ...
    def get_expert_model_parallel_world_size(self):
        ...
        if self._MPU_EXPERT_MODEL_PARALLEL_WORLD_SIZE:
            return self._MPU_EXPERT_MODEL_PARALLEL_WORLD_SIZE
        if torch.distributed.is_available() and torch.distributed.is_initialized():
    ...
    def get_expert_model_parallel_rank(self):
        ...
        if self._MPU_EXPERT_MODEL_PARALLEL_RANK:
            return self._MPU_EXPERT_MODEL_PARALLEL_RANK
        if torch.distributed.is_available() and torch.distributed.is_initialized():
