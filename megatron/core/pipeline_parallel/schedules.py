...
def fwd(
    ...
    if config.timers is not None:
        raise NotImplementedError("forwards and backwards are now run separately.")
        # config.timers('forward-backward', log_level=1).start(barrier=config.barrier_with_L1_time)
    ...
    # Compute number of warmup microbatches.
    num_warmup_microbatches = (
        parallel_state.get_f_pipeline_model_parallel_world_size()
        - parallel_state.get_f_pipeline_model_parallel_rank()
        - 1
    )
    num_warmup_microbatches = min(num_warmup_microbatches, num_microbatches)
    num_microbatches_remaining = num_microbatches - num_warmup_microbatches
    ...
    rank = parallel_state.get_f_pipeline_model_parallel_rank()
    recv_tensor_shapes = get_tensor_shapes(
        rank=rank - 1,
        model_type=model_type,
        seq_length=seq_length,
    ...
    #send these to bGPUs for recomputation
    input_tensors = []
    # if not forward_only:
    #     input_tensors = []
    #     output_tensors = []
